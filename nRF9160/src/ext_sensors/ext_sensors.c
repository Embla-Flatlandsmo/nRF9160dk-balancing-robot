...
#if defined(CONFIG_MPU9250_ACCEL_RANGE_2G)
#define MPU9250_MAX_M_S2 2*9.80665
#elif defined(CONFIG_MPU9250_ACCEL_RANGE_4G)
#define MPU9250_MAX_M_S2 4 * 9.80665
#elif defined(CONFIG_MPU9250_ACCEL_RANGE_8G)
#define MPU9250_MAX_M_S2 8 * 9.80665
#elif defined(CONFIG_MPU9250_ACCEL_RANGE_16G)
#define MPU9250_MAX_M_S2 16 * 9.80665
#endif
...
static struct env_sensor gyro_sensor = {
	.channel = SENSOR_CHAN_GYRO_XYZ,
	.dev = DEVICE_DT_GET(DT_ALIAS(gyroscope)),
};
...
int ext_sensors_init(ext_sensor_handler_t handler)
{
	...
	if (!device_is_ready(gyro_sensor.dev)) {
		LOG_ERR("Gyroscope sensor device is not ready");
		evt.type = EXT_SENSOR_EVT_GYROSCOPE_ERROR;
		evt_handler(&evt);
	}
